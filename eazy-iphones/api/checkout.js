...
module.exports = async function handler(req, res) {
  ...
  const { data: phone, error } = await sb.from('phones').select('*').eq('id', phoneId).single()
  ...
  const description = [phone.battery ? `${phone.battery}% battery` : null, phone.condition, phone.network].filter(Boolean).join(' · ')
  const origin = process.env.SITE_URL || `https://${req.headers.host}`

  const session = await stripe.checkout.sessions.create({
    payment_method_types: ['card'],
    line_items: [{ price_data: { currency: 'gbp', product_data: { name: productName, description: description || undefined }, unit_amount: Math.round(phone.price * 100) }, quantity: 1 }],
    mode: 'payment',
    success_url: `${origin}/success.html?session_id={CHECKOUT_SESSION_ID}`,
    cancel_url: `${origin}/buy.html`,
    metadata: { phone_id: String(phoneId), phone_name: productName },
